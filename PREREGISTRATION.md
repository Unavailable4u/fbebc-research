...
| Selection rule | As specified in the header comment of `delta/selection.py` — top-k distinct (full canonical-AST hash; **key amended by deviation D2** — the original node-type-only hash was blind to constants and names) attested-valid candidates ever seen, tie-inclusive, parent sampled uniformly from the band. k = 1 reproduces the pre-existing single-winner rule. **Not tuned.** k = 3 is chosen a priori and is the only k run. |
| Arm scheduling | All 6 arms advance in chunks of `--round-size` = 10 generations so any early stop leaves the arms matched to within one chunk. **[Mechanism amended after launch — see deviation D1: chunks now always go to the least-progressed arm. Intent unchanged.]** |
...
|---|---|---|---|---|
| Week 3 Day 17 (date = that commit's timestamp) | the commit containing this row (`git log -- PREREGISTRATION.md`) | **D2: duplicate key for the elite-band/single-winner rule changed from `semantic_fingerprint` to a new `exact_fingerprint`** (`delta/admission/metrics.py`, `delta/selection.py`, `delta/loop.py`; the frozen distance metric and the ledger's `semantic_fingerprint` column are unchanged). | **Implementation bug in the pre-registered rule.** `semantic_fingerprint` hashes only AST node *types*, so two programs differing only in numeric constants or names were treated as duplicates, and a constant-tuned child was rejected as a "duplicate" **even when strictly fitter** (both arms, k=1 included). The rule's *intent* — reject semantically identical rewrites — is unchanged; the key now matches that intent (layout, comments and docstrings ignored; constants and names count). Found by inspecting the first real step-size data and confirmed by running the metric (`1.0→100.0` and `side→kk` both had distance 0 and an identical fingerprint); missed earlier because the tests used fakes whose fingerprints I controlled. | **Nothing in the data collected so far.** `scripts/replay_selection.py` replayed all six arms' ledgers (63 generations; 6–10 candidates per arm reached selection) through the rule as run and the corrected rule, using the recorded candidate sources: each replay reproduced its arm's checkpoint band exactly, and **no candidate was decided differently in any arm**. Saved as `runs/week3.replay_d2.txt`. From the fix onward constant-tuned improvements can enter the band, and `runmeta.json` will show a third git commit across invocations (launch, D1, D2) — expected. |
| 2026-09-24 | the commit containing this row (`git log -- PREREGISTRATION.md`); launch was at tag `prereg-week3` | **D1: arm scheduler changed from per-pass round-robin to least-progressed-first** (`scripts/run_stage1.py`: `plan_round` → `next_chunk`). | The first real launch stopped at the daily token cap with arms at [13, 10, 10, 10, 10, 10] generations. A per-pass round-robin that restarts from the first arm on every resume lets early arms gain an extra chunk whenever a daily stop lands mid-pass, and the lead accumulates; a regression test reproduced arms at [10, 10, 30, 30, 30, 33] by day 2 under the old scheduler. That would have defeated the matched-arm design. Found by inspecting the counts before the second day's run. | **Only the order in which arms receive quota.** No arm's selection rule, parent-sampling RNG, prompt, evaluation, scorer, or seed changed, and Σ's behavior does not depend on scheduling order, so each arm's own trajectory is unaffected. `runmeta.json` will show two git commits across invocations (launch, then this fix) — expected. The launch-day state was single_winner/seed0 = 13 generations, all other arms = 10 (a 3-generation offset that persists; analysis is at G_common regardless). |
